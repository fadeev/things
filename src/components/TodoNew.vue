<template>
  <div :class="{new: true, full: full, compact: !full, selected: selected}" @click="select" @dblclick="fullToggle">
    ...
    <div :class="{anim: true, on: full, off: !full}">
    <div class="row notes">
        ...
        <div style="position: relative">
          ...
          <transition name="fade-in">
            <div class="menu" v-if="tag && tagsStore.length > 0">
              <div class="item" @click.stop="addTag(tag)" v-for="(tag, index) in tagsStore" :key="index">
      ...
      <div :class="{button: true, active: active === 'date'}" v-if="!date" @click.stop="activate('date')">
        ...
        <transition name="fade-in">
          <div class="menu" v-if="active === 'date'">
            <div class="item" @click.stop="setDate('Today')">
...
<style scoped>
  * { font-family: "Roboto", sans-serif;}
  .anim { transition: all .1s ; }
  .anim.off { max-height: 0; opacity: 0; overflow: hidden; }
  .anim.on { max-height: 300px; opacity: 1; }
  .done { transition: all .5s; }
  .done:active { transform: scale(1.2); }
  .new { transition: all .5s, background 0s; padding: 7px 20px; margin-left: 40px; margin-right: 40px; }
  .new.selected { background: rgba(0, 81, 255, .15); border-radius: 5px; }
  .new.compact { cursor: pointer; user-select: none; }
  .new.full { background: white; box-shadow: 0 1px 3px 1px rgba(0,0,0,.1); padding: 20px; margin-left: 40px; margin-right: 40px; margin-bottom: 20px; margin-top: 20px; border-radius: 5px; }
  ...
  .new .row.parameters { display: flex; justify-content: flex-end; align-content: center; margin-bottom: 0; margin-top: 30px; }
  ...
  .checklist .item { display: flex; align-items: center; border-top: 1px solid rgba(0,0,0,.1); padding: 7px; }
  ...
  export default {
    ...
    methods: {
      ...
      tagCreate(tag) {
        ...
        if (!exist) store.commit("tagCreate", tag)
        if (!already) this.addTag(tag)
      },
      tagDelete(e) {
      ...
      checklistCreate(e) {
        ...
        this.data.checklist.push({name: e.key})
        this.$nextTick(() => {
          this.$refs.checklistItem[0].focus()
        })
      },
      checklistItemNew() {
        this.data.checklist = this.data.checklist || []
        this.data.checklist.push({name:null})
        this.$nextTick(() => {
      ...
      select() {
        ...
        if (this.data.done !==  false && this.data.done !== true) {
          this.$set(this.data, "done", null)
        }
